...
public class ServiceConfigPropertiesUtil {
    ...
    private ServiceConfigPropertiesUtil() {
        configFilePath = ServiceMain.getRootDir() + File.separator + "conf" + File.separator
    ...
    public long getBackupStartupDelay() {
        ...
        try {
            ...
            if (temLongVal < 0 || temLongVal > 1000 * 60 * 60 * 24) {
                logger.warn("Backup startup delay time is invalid, " + temLongVal
                                + ", use default: " + longVal);
            } else {
                longVal = temLongVal * 1000;
            }
        } catch (NumberFormatException e) {
            logger.error("NumberFormatException occurred when getBackupStartupDelay.", e);
        }
        return longVal;
    }
    ...
    public long getBackupInterval() {
        ...
        try {
            ...
            if (temLongVal < 0 || temLongVal > 1000 * 60 * 60 * 24 * 365) {
                logger.warn("Backup interval time is invalid, " + temLongVal + ", use default: "
                                + longVal);
            } else {
                longVal = temLongVal * 1000;
            }
        } catch (NumberFormatException e) {
            logger.error("NumberFormatException occurred when getBackupInterval.", e);
        }
        return longVal;
    }

...
public class BackupTask extends TimerTask {
	...
	private void executeBackupTask(BackupTaskController controller) {
		List<String> sourceKeys = BackupConfigPropertiesUtil.getInstance()
				.getAllSourceKeys();
		for (String sourceKey : sourceKeys) {
			String destKey = BackupConfigPropertiesUtil.getInstance()
					.getDestKeyBySourceKey(sourceKey);
			String sourceDir = BackupConfigPropertiesUtil.getInstance()
					.getValue(sourceKey);
			String destDir = BackupConfigPropertiesUtil.getInstance().getValue(
					destKey);
			if (null == sourceDir || "".equals(sourceDir)) {
				logger.warn("sourceDir is null or empty, sourceDir: "
						+ sourceDir + ", destDir: " + destDir);
				continue;
			}

			if (null == destDir || "".equals(destDir)) {
				logger.warn("destDir is null or empty, sourceDir: " + sourceDir
						+ ", destDir: " + destDir);
				continue;
			}

			File sourceFile = new File(sourceDir);
			File destFile = new File(BackupConfigPropertiesUtil.getInstance()
					.getValue(destKey));
	...
	private void processBackupTask(String sourceDir, String destDir,
		...
		for (File file : files) {
			File newDestFile = new File(destDir + File.separator
					+ file.getName());
			if (file.isFile()) {
				if (!newDestFile.exists()) {
					FileEncryptionExecutor.getInstance().submitTask(
							new FileEncryptionThread(file.getAbsolutePath(),
				...
				processBackupTask(file.getAbsolutePath(),
						newDestFile.getAbsolutePath(), controller);
			}
		}
	}
